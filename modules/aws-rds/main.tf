...
resource "aws_db_instance" "default" {
    ...
    engine                = local.is_replica ? null : var.engine_version
    ...
    final_snapshot_identifier   = length(var.final_snapshot_identifier) > 0 ? var.final_snapshot_identifier : module.snapshot_label.id
...
resource "aws_db_option_group" "default" {
  ...
  dynamic "option" {
    ...
    content {
      ...
      port = lookup(option.value, "version", null)
...
module "dns" {
    ...
    records  = coalesce(aws_db_instance.default.*.address, [""])
