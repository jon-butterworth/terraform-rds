...
resource "vault_generic_secret" "dmba-secret" {
  ...
  data_json = jsonencode({
    database = module.dmba.db_name
    host     = module.dmba.address
    password = random_password.dmba-pwd.result
    username = module.dmba.username
  })
}
...
module "dmba" {
  ...
  identifier           = local.dmba["identifier"]
  database_name        = local.dmba["name"]
  database_user        = local.dmba["user"]
  kms_key_arn          = module.dmba-kms.key_arn
  database_password    = random_password.dmba-pwd.result
  ...
  db_parameter = local.dmba["parameters"]
  ...
  rules = [
    {
      ...
      description = "Allow ${local.global["db_port"]} for dmba"
    },
  ]
}